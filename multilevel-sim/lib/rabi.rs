...
pub mod schrodinger {
    ...
    /// Compute the Schrödinger coherent evolution of the initial state `psi0`
    /// for Hamiltonian `H`.
    ///
    /// Note: `psi0` should be the initial state at time `t = 0`, not
    /// necessarily any of the elements of `t`.
    pub fn eigen(
        psi0: &nd::Array1<C64>,
        H: &nd::Array2<C64>,
        t: &nd::Array1<f64>,
    ...
    ///
    /// Fails if a time-independent Hamiltonian cannot be built or the initial
    /// state description is invalid.
    ...
    /// Compute the Schrödinger coherent evolution of an initial state `psi0`
    /// for time-dependent Hamiltonian `H` by diagonalizing at each time step.
    pub fn eigen_t(
        psi0: &nd::Array1<C64>,
        H: &nd::Array3<C64>,
        t: &nd::Array1<f64>,
