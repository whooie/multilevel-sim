...
outdir = Path("output")
infile = outdir.joinpath("ququart_clock_corpse.npz")

data = np.load(str(infile))
rabi_freq = data["rabi_freq"][0]
time = data["time"]
...
k_pipulse = len(time) - 1

P = pd.Plotter()
for pt in pulse_times:
    P.axvline(pt, linestyle="--", color="k")
(
    P
    ...
    .savefig(outdir.joinpath("ququart_clock_corpse_probs.png"))
)
...
(
    ...
    .set_xlabel("Time [μs]")
    .set_ylabel("$\\bar{n}$")
    .savefig(outdir.joinpath("ququart_clock_corpse_nbar.png"))
)
...
print(f"change in nbar      = {dnbar:.6f}")

print(
"["
+ f"{rabi_freq:.3f}, "
+ f"{clock_population:.3f}, "
+ f"{purity:.6f}, "
+ f"{purity_o:.6f}, "
+ f"{purity_n:.6f}, "
+ f"{purity_ground:.6f}, "
